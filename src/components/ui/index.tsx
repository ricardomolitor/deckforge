  ...
  return (
    <div className="space-y-1">
      ...
      <input
        id={inputId}
        className={clsx(
          'w-full rounded-lg border px-3 py-2 text-sm text-gray-900 placeholder-gray-400 transition-colors focus:outline-none focus:ring-2 focus:ring-brand-500/20',
          error ? 'border-red-300 focus:border-red-500' : 'border-gray-300 focus:border-brand-500',
          className
        )}
        aria-invalid={!!error}
  ...
  return (
    <div className="space-y-1">
      ...
      <select
        id={inputId}
        className={clsx(
          'w-full rounded-lg border px-3 py-2 text-sm text-gray-900 transition-colors focus:outline-none focus:ring-2 focus:ring-brand-500/20',
          error ? 'border-red-300 focus:border-red-500' : 'border-gray-300 focus:border-brand-500',
          className
        )}
        aria-invalid={!!error}
...
export function Panel({ open, onClose, title, children, size = 'md', footer }: PanelProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className={clsx('relative w-full bg-white shadow-xl panel-content flex flex-col', sizes[size])}>
        {/* Header */}
        <div className="flex items-center justify-between border-b border-gray-200 px-6 py-4">
          <h2 className="text-lg font-semibold text-gray-900">{title}</h2>
          <button
            onClick={onClose}
            className="rounded-md p-1.5 text-gray-400 hover:bg-gray-100 hover:text-gray-600 focus:outline-none focus:ring-2 focus:ring-brand-500"
...
export function Dialog({ open, onClose, title, children, footer }: DialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md rounded-xl bg-white shadow-xl animate-fade-in">
...
export function EmptyState({ icon, title, description, action }: EmptyStateProps) {
  return (
    <div className="flex flex-col items-center justify-center rounded-lg border-2 border-dashed border-gray-200 px-8 py-16 text-center">
